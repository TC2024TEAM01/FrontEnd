...
<feBlend mode="normal" in="SourceGraphic" in2="effect3_dropShadow_69_2892" result="shape"/>
        ...
        <Link to="/assignments" className="navbar-link">Dashboard</Link>
          <div className="navbar-dropdown">
            <button className="navbar-link navbar-button" onClick={toggleDropdown}>
              User
            </button>
            {dropdownVisible && (
              <div className="dropdown-menu">
                <Link to="/user-stats" className="dropdown-item">List Of Users</Link>
                <Link to="/check-in-out" className="dropdown-item">Add User</Link>
              </div>
            )}
          </div>
          <Link to="/vacations" className="navbar-link">Face Recognition</Link>
        </div>          
      </div>
    </nav>
  );
